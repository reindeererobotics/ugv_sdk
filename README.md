...
## Updating fork with source(westonrobot/ugv_sdk)
1. Branch off from the `dev` branch and name the branch `chore/merge-weston-changes`
2. Create a pull request with `westonrobot/ugv_sdk/main` [repo](https://github.com/westonrobot/ugv_sdk/tree/main). Use the `compare across forks` to verify the branches that are being merged.
### Testing
1. Navigate to `dasher_ros2` and checkout the `dev` branch on the robot
2. Update the following `GIT_TAG` in the fetchcontent module so as your branch may be fetched and tested on the robot. i.e.
```diff
FetchContent_Declare(
  ugv_sdk
  GIT_REPOSITORY https://github.com/reindeererobotics/ugv_sdk.git
- GIT_TAG dev
+ GIT_TAG chore/merge-weston-changes
...
```
3. Follow the steps in `dasher_ros2` to build and launch `dasher_base`
4. Document any breaking changes and create an issue in westonrobots [issues](https://github.com/westonrobot/ugv_sdk/issues) page
